...
export function processCandidate(candidateData) {
  const data = []
...
export function getContributorData(zipcode, testing) {
  let url
  testing ? url = 'https://localhost:3500/api/data/CandidateSummary/' : '/api/data/CandidateSummary/'
  return dispatch => {
    return fetch(url + zipcode + '/2016')
